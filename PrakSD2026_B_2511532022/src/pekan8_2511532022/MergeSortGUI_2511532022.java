...
public class MergeSortGUI_2511532022 extends JFrame {
	...
	private Queue<int[]> mergeQueue_2022 = new LinkedList<>();
	private int stepCount_2022 = 1;
	private boolean isMerging_2022 = false;
	private boolean copying_2022 = false;

	private int[] temp_2022;
	private int left_2022, mid_2022, right_2022;
	private int i_2022, j_2022, k_2022;

	public MergeSortGUI_2511532022() {
	...
	private void setArrayFromInput_2022() {
		...
		mergeQueue_2022.clear();
		generateMergeSteps_2022(0, array_2022.length - 1);
		stepButton_2022.setEnabled(true);
		stepArea_2022.setText("");
		stepCount_2022 = 1;
		isMerging_2022 = false;

		panelArray_2022.revalidate();
		panelArray_2022.repaint();
	}

	private void generateMergeSteps_2022(int left, int right) {
		if (left >= right) return;
		int mid = (left + right) / 2;
		generateMergeSteps_2022(left, mid);
		generateMergeSteps_2022(mid + 1, right);
		mergeQueue_2022.add(new int[]{left, mid, right});
	}

	private void performStep_2022() {
		resetHighlights_2022();

		if (!isMerging_2022 && !mergeQueue_2022.isEmpty()) {
			int[] range_2022 = mergeQueue_2022.poll();
			left_2022 = range_2022[0];
			mid_2022 = range_2022[1];
			right_2022 = range_2022[2];
			temp_2022 = new int[right_2022 - left_2022 + 1];
			i_2022 = left_2022;
			j_2022 = mid_2022 + 1;
			k_2022 = 0;
			copying_2022 = false;
			isMerging_2022 = true;
			stepArea_2022.append("Langkah " + stepCount_2022++ +
					": Mulai merge dari " + left_2022 + " ke " + right_2022 + "\n");
			return;
		}

		if (isMerging_2022 && !copying_2022) {
			if (i_2022 <= mid_2022 && j_2022 <= right_2022) {
				labelArray_2022[i_2022].setBackground(Color.CYAN);
				labelArray_2022[j_2022].setBackground(Color.CYAN);
				if (array_2022[i_2022] <= array_2022[j_2022]) {
					temp_2022[k_2022++] = array_2022[i_2022++];
				} else {
					temp_2022[k_2022++] = array_2022[j_2022++];
				}
				stepArea_2022.append("Langkah " + stepCount_2022++ + ": Bandingkan dan salin elemen\n");
				return;
			} else if (i_2022 <= mid_2022) {
				temp_2022[k_2022++] = array_2022[i_2022++];
				stepArea_2022.append("Langkah " + stepCount_2022++ + ": Salin sisa kiri\n");
				return;
			} else if (j_2022 <= right_2022) {
				temp_2022[k_2022++] = array_2022[j_2022++];
				stepArea_2022.append("Langkah " + stepCount_2022++ + ": Salin sisa kanan\n");
				return;
			} else {
				copying_2022 = true;
				k_2022 = 0;
		...
		if (copying_2022 && k_2022 < temp_2022.length) {
			array_2022[left_2022 + k_2022] = temp_2022[k_2022];
			labelArray_2022[left_2022 + k_2022].setText(String.valueOf(temp_2022[k_2022]));
			labelArray_2022[left_2022 + k_2022].setBackground(Color.GREEN);
			k_2022++;
			stepArea_2022.append("Langkah " + stepCount_2022++ + ": Tempelkan ke array utama\n");
			return;
		}
		...
		if (mergeQueue_2022.isEmpty() && !isMerging_2022) {
			stepArea_2022.append("Selesai.\n");
	...
	private void resetHighlights_2022() {
		if (labelArray_2022 == null) return;
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> {
			MergeSortGUI_2511532022 gui = new MergeSortGUI_2511532022();
			gui.setVisible(true);
		});
	}
}